...
BAG_WILD = glob_wildcards(INPUT_DIR + "/{vin,.{17}}/bagfiles/{day_folder,.{10}}/{name}.{ext, bag|bag.active|csv}")
...
rule copy:
    ...
    output:
        BUILD_DIR + "/{vin}/bagfiles/{day_folder}/{name}.{ext, bag|bag.active|csv}"
    shell:
        """
        cp {input} {output}
        """